                        ...
                        <div className="flex flex-col h-full space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-sm text-gray-700 border-b border-gray-200 pb-4">
                                <div className="flex items-center">
                                    ...
                                    <span>Created By: <span className="font-semibold">{selectedTicket.createdBy}</span></span>
                                </div>
                                <div className="flex items-center">
                                    <Calendar className="w-4 h-4 mr-2 text-gray-500" />
                                    <span>Created On: <span className="font-semibold">{new Date(selectedTicket.createdAt).toLocaleDateString()}</span></span>
                                </div>
                                <div className="flex items-center">
                                    <Clock className="w-4 h-4 mr-2 text-gray-500" />
                                    <span>Status: <span className={`font-semibold px-2 py-0.5 rounded-full ${getStatusColor(selectedTicket.status)}`}>{selectedTicket.status}</span></span>
                                </div>
                                ...
                                {user?.role === 'customer' && selectedTicket.assignedTo && (
                                    <div className="flex items-center">
                                        <UserCheck className="w-4 h-4 mr-2 text-gray-500" />
                                        <span>Assigned To: <span className="font-semibold">{selectedTicket.assignedTo}</span></span>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="border-b border-gray-200 pb-4">
                                ...
                                <p className="text-gray-800 text-sm leading-relaxed">{selectedTicket.description}</p>
                            </div>

                            {/* Chat Button (Now navigates to new route) */}
                            <div className="flex justify-center items-center py-8">
                                <button
                                    onClick={() => navigate(`/tickets/${selectedTicket._id}/chat`)} // Navigate to new chat route
